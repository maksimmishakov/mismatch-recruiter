...
const BatchPage = () => {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <button onClick={() => setUploadMode('csv')} className={`p-4 rounded-lg border-2 transition ${uploadMode === 'csv'
            ...
            <button onClick={() => setUploadMode('zip')} className={`p-4 rounded-lg border-2 transition ${uploadMode === 'zip'
            ? 'border-purple-500 bg-purple-50'
            : 'border-gray-300 hover:border-gray-400'}`}>
          ...
          {/* Template Download */}
          {uploadMode === 'csv' && (<div className="mb-6">
              <button onClick={downloadTemplate} className="inline-flex items-center gap-2 text-purple-600 hover:text-purple-700 font-medium">
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center mb-6">
            <input type="file" onChange={handleFileChange} accept={uploadMode === 'csv' ? '.csv' : '.zip'} className="hidden" id="batch-file-input" disabled={processing}/>
          ...
          {/* Selected File Info */}
          {batchFile && (<div className="p-4 bg-purple-50 border border-purple-200 rounded-lg mb-6">
          ...
          {/* Error Message */}
          {error && (<div className="p-4 bg-red-50 border border-red-200 rounded-lg flex items-start gap-3 mb-6">
              <AlertCircle className="h-5 w-5 text-red-600 flex-shrink-0 mt-0.5"/>
              <p className="text-sm text-red-700">{error}</p>
            </div>)}

          {/* Results */}
          {batchResult && (<div className="p-4 bg-blue-50 border border-blue-200 rounded-lg mb-6">
              ...
              {batchResult.results.length > 0 && (<div className="mt-4 border-t pt-4">
                  <h3 className="font-medium text-gray-900 mb-2">Details:</h3>
                  <div className="max-h-64 overflow-y-auto space-y-2">
                    {batchResult.results.map((result, idx) => (<div key={idx} className="flex items-start gap-2 text-sm">
                        {result.status === 'success' ? (<CheckCircle className="h-4 w-4 text-green-600 flex-shrink-0 mt-0.5"/>) : (<AlertCircle className="h-4 w-4 text-red-600 flex-shrink-0 mt-0.5"/>)}
                        <div>
          ...
          {/* Upload Button */}
          <button onClick={handleBatchUpload} disabled={!batchFile || processing} className={`w-full py-3 px-4 rounded-lg font-medium text-white transition flex items-center justify-center gap-2 ${batchFile && !processing

...
export default function MobileMenu() {
    ...
    return (<>
      <button onClick={toggleMenu} className="md:hidden fixed top-4 left-4 p-2 bg-gray-900 text-white rounded z-50" aria-label="Toggle menu">
        {isOpen ? '×' : '☰'}
      </button>

      {isOpen && (<div className="md:hidden fixed inset-0 bg-black bg-opacity-50 z-40" onClick={toggleMenu}/>)}

      <nav className={`md:hidden fixed left-0 top-0 h-full w-64 bg-gray-900 text-white transform transition-transform z-50 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
